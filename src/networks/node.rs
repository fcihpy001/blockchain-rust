...
use crate::networks::command::{Commands, Messages};
use crate::networks::swarm::{BLOCK_TOPIC, create_swarm, PEER_ID, TRANX_TOPIC, WALLET_MAP};
...
impl<T: Storage> Node<T> {
    pub async fn new(storage: Arc<T>) -> Result<Self> {
        ...
        Ok(Self {
            ...
            msg_receiver,
            swarm: create_swarm(
                vec![BLOCK_TOPIC.clone(),TRANX_TOPIC.clone()],
                msg_sender
            ).await?
        })
    }
